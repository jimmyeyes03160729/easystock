...
(async()=>{
 ...
 assert.equal(w.document.querySelectorAll('#conversationList input[type=checkbox]').length,3);
 ...
 assert.deepEqual(JSON.parse(writes[0].options.body),{push:false,version:1});
