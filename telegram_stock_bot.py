"""EasyStock Telegram Bot long poller (Groups and 1-on-1 direct chats)."""
import hashlib
import json
import os
...
def api(method, data, files=None):
    if method not in ('getUpdates', 'getWebhookInfo', 'sendMessage', 'sendPhoto', 'answerCallbackQuery'):
...
def keyboard(message):
    ...
    for item in (contents.get('footer') or {}).get('contents', []):
        ...
        if action.get('type') == 'message':
            ...
            if len(query.encode()) <= 64:
                buttons.append({'text': str(action.get('label') or '查詢'), 'callback_data': query})
...
def image_bytes(message):
    ...
    if path.parent != root or path.is_symlink() or not 0 < path.stat().st_size <= 10 * 1024 * 1024:
        raise ValueError('Invalid chart')
...
def send_messages(store, chat_id, messages, reply_id=None, is_private=False):
    sent = 0
    for message in messages[:5]:
        if not enabled(store, chat_id, is_private=is_private):
            break
        data = {'chat_id': str(chat_id)}
        if reply_id:
            data['reply_parameters'] = {'message_id': reply_id, 'allow_sending_without_reply': True}
        if message.get('type') == 'text':
            text = str(message.get('text') or '')
            for start in range(0, min(len(text), 8192), 4096):
                if not enabled(store, chat_id, is_private=is_private):
                    break
                api('sendMessage', {**data, 'text': text[start:start+4096], 'link_preview_options': {'is_disabled': True}})
                sent += 1
            ...
            if message.get('altText'):
                data['caption'] = str(message['altText'])[:1024]
            data = {key: json.dumps(value) if isinstance(value, dict) else value for key, value in data.items()}
            api('sendPhoto', data, files={'photo': ('chart.png', content, 'image/png')})
            sent += 1
    return sent
...
def handle_update(store, update):
    ...
    if is_private:
        if hasattr(store, 'is_private_replies_enabled') and not store.is_private_replies_enabled():
            return
    elif chat_type in ('group', 'supergroup'):
        if str(chat.get('id')) != group or not enabled(store, group, is_private=False):
            return
    else:
        return
    ...
    if callback:
        ...
        if not isinstance(text, str) or not text.startswith('q:'):
            return
        text = text[2:]
        try:
        ...
        if not isinstance(message.get('date'), (int, float)) or not -30 <= time.time() - message['date'] <= 120:
            return
        if not isinstance(text, str):
            return
        raw_text = text.strip()
        # 處理加入好友 / 首次啟動 (/start) 自動回覆
        if is_private and raw_text == '/start':
            auto_reply = store.is_auto_reply_on_follow_enabled() if hasattr(store, 'is_auto_reply_on_follow_enabled') else True
            if auto_reply:
                welcome_text = (
                    "👋 歡迎使用 EasyStock 智慧量化看盤機器人！\n\n"
                    "您可以直接傳送股票代碼或指令查詢：\n"
                    "📈 P2330 或 P台積電：分時走勢與五檔\n"
                    "🕯️ K2330 或 K台積電：日 K 線圖與技術指標\n"
                    "🏛️ T2330 或 T台積電：三大法人買賣超\n"
                    "💰 D2330 或 D台積電：股利與殖利率\n"
                    "📊 #大盤 / P大盤 / K大盤：加權指數走勢\n"
                    "⚡ 早報 / 當沖：即時摘要情報\n"
                    "❓ 輸入「指令」可隨時查看完整說明。"
                ...
                try:
                    api('sendMessage', {'chat_id': chat_id, 'text': welcome_text, 'link_preview_options': {'is_disabled': True}})
                except Exception:
                    pass
            return
    ...
    try:
        response = requests.post(
            'http://127.0.0.1:8088/internal/telegram-query',
            headers={'X-Telegram-Bridge': bridge_secret()},
            json={'group': chat_id, 'text': text.strip(), 'is_private': is_private},
            timeout=(3, 100),
        )
        if response.status_code not in (200, 503):
            return
        messages = response.json().get('messages', [])
        sent = send_messages(store, chat_id, messages, message.get('message_id'), is_private=is_private)
        print(f'[TELEGRAM QUERY] completed; chat={chat_id} is_private={is_private} messages={sent}', flush=True)
    except Exception:
