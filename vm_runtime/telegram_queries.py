...
HELP='''📊 EasyStock Telegram 群組查詢
#台積電 / #2330 / #大盤：查價
P台積電 / P2330 / P大盤：當日走勢圖
K台積電 / K2330 / K大盤：日 K 圖
T台積電 / T2330 / T大盤：三大法人
D台積電 / D2330：股利與殖利率
當沖 / 早報：查閱最新摘要
指令：顯示本說明
圖卡按鈕可切換 K 線、即時走勢、法人資料。
僅支援指定群組，私人對話關閉。查詢與進出場通知可於後台分別開關。'''
...
def enabled(store,group):
    token,target=telegram_config()
    if not token or str(group)!=target:return False
    with store.tx() as db:
...
def register_bridge(app,store,parse,handle):
    ...
    def telegram_query():
        ...
        if request.remote_addr not in ('127.0.0.1','::1') or not secret or not hmac.compare_digest(request.headers.get('X-Telegram-Bridge',''),secret):
            return '',403
        if not request.is_json or not request.content_length or request.content_length>4096:return '',400
        data=request.get_json(silent=True)
        if not isinstance(data,dict) or set(data)!={'group','text'} or not isinstance(data['text'],str) or len(data['text'])>160:return '',400
        if not enabled(store,data['group']):return jsonify(messages=[])
        cmd=parse(data['text'])
        if cmd is None:return jsonify(messages=[])
        try:
            with store.tx() as db:store._limit(db,'telegram-query',15)
        except Denied:return jsonify(messages=[]),429
        try:
            if cmd.kind in ('HELP','NOTIFY_INFO','UPDATE'):messages=[{'type':'text','text':HELP}]
            else:messages=handle(cmd)
            ...
            for message in messages or []:
                ...
                safe.append(message)
            return jsonify(messages=safe[:5] if enabled(store,data['group']) else [])
        except Exception:
