...
def initialize(db):
    ...
    if 'archived' not in {r[1] for r in db.execute('PRAGMA table_info(line_conversations)')}:
        db.execute('ALTER TABLE line_conversations ADD COLUMN archived INTEGER NOT NULL DEFAULT 0')
...
def observe(store, source, label=None):
    if source.get('type') != 'group': return None
    item = identity(source)
    if not item: return None
    key, sid, kind = item
    title = (label or ('個人' if kind == 'user' else '群組') + ' …' + sid[-6:])[:80]
    with store.tx() as db:
        db.execute('INSERT OR IGNORE INTO line_conversations(id,source_id,kind,label,seen,archived) VALUES(?,?,?,?,?,1)',
...
def state(store):
    with store.tx() as db:
        ...
        rows = db.execute("SELECT id,kind,label,replies,push,seen,version FROM line_conversations WHERE kind='group' AND archived=0 ORDER BY seen DESC LIMIT 500").fetchall()
...
def update(store, data):
    ...
    if data['values']['users'] or data['values']['other_push']:
        raise ValueError('個人功能與其他自動推播已停用。')
    with store.tx() as db:
...
def update_conversation(store, key, data):
    ...
    with store.tx() as db:
        result = db.execute("UPDATE line_conversations SET replies=?,push=?,label=?,version=version+1 WHERE id=? AND version=? AND kind='group' AND archived=0",
...
def allowed(store, source, category='reply'):
    if source.get('type') != 'group' or category not in ('reply','trade'): return False
    item = identity(source)
    ...
    with store.tx() as db:
        ...
        row = db.execute('SELECT replies,push FROM line_conversations WHERE id=? AND archived=0',(key,)).fetchone()
    # Only explicitly approved groups may receive replies or notifications.
    if category == 'reply':
        return bool(row and row[0]) and preferences['replies'] and preferences['groups']
