...
def initialize(db):
    db.executescript('''
    CREATE TABLE IF NOT EXISTS telegram_policy(id INTEGER PRIMARY KEY CHECK(id=1),push INTEGER NOT NULL DEFAULT 0,version INTEGER NOT NULL DEFAULT 1,replies INTEGER NOT NULL DEFAULT 0);
    ...
    ''')
    if 'replies' not in {r[1] for r in db.execute('PRAGMA table_info(telegram_policy)')}:
        db.execute('ALTER TABLE telegram_policy ADD COLUMN replies INTEGER NOT NULL DEFAULT 0')
        db.execute('UPDATE telegram_policy SET version=version+1 WHERE id=1')
...
def state(store):
    ...
    with store.tx() as db:
        push, version, replies = db.execute('SELECT push,version,replies FROM telegram_policy WHERE id=1').fetchone()
...
def update(store, platform, key, data):
    ...
    if platform == 'telegram' and 'replies' in data: required.add('replies')
    ...
    with store.tx() as db:
        if platform == 'telegram':
            ...
            if (data['push'] or data.get('replies')) and not all(telegram_config()): raise ValueError('Telegram 尚未設定完成。')
            result = db.execute('UPDATE telegram_policy SET push=?,replies=COALESCE(?,replies),version=version+1 WHERE id=1 AND version=?',
                                (data['push'], data.get('replies'), data['version']))
        else:
