...
【EPS 獲利 E】
E台積電 / E2330 / EPS台積電 / EPS2330
近 8 季 EPS 柱狀圖、近四季累計 TTM、本益比、毛利率
...
def parse_command(text: str) -> ParsedCommand | None:
    ...
    # EPS 獲利查詢：EPS台積電 / 台積電EPS / EPS2330 / 2330EPS
    if upper.startswith("EPS") and len(compact) > 3:
        query = compact[3:]
        if query and query != "大盤":
            return ParsedCommand("E", query, raw)

    if upper.endswith("EPS") and len(compact) > 3:
        query = compact[:-3]
        if query and query != "大盤":
            return ParsedCommand("E", query, raw)

    if upper[0:1] in {"P", "K", "T", "D", "E"} and len(compact) > 1:
        kind = upper[0]
...
def fetch_stock_eps_data(code: str) -> dict | None:
    end = now_tpe().date()
    ...
    gross_dict: dict[str, float] = {}
    op_dict: dict[str, float] = {}
    net_dict: dict[str, float] = {}

    for r in rows:
        t = str(r.get("type") or "")
        orig = str(r.get("origin_name") or "")
        d = str(r.get("date") or "")
        val = _num(r.get("value"))
        if val is None or not d:
            continue

        if "EPS" in t.upper() or "每股盈餘" in orig:
            eps_dict[d] = val
        elif t == "Revenue" or "營業收入" in orig:
            rev_dict[d] = val
        elif t == "GrossProfit" or "營業毛利" in orig:
            gross_dict[d] = val
        elif t == "OperatingIncome" or "營業利益" in orig:
            op_dict[d] = val
        elif t == "IncomeAfterTaxes" or "稅後淨利" in orig:
            net_dict[d] = val

    sorted_dates = sorted(eps_dict.keys())
    if not sorted_dates:
        return None

    def _to_quarter(dt_str: str) -> str:
        parts = dt_str.split("-")
        yr = parts[0][2:]
        m = int(parts[1])
        q = (m - 1) // 3 + 1
    ...
    for d in sorted_dates:
        quarterly.append({
            "date": d,
            "quarter": _to_quarter(d),
            "eps": eps_dict[d],
            "rev": rev_dict.get(d),
            "gross": gross_dict.get(d),
            "op": op_dict.get(d),
            "net": net_dict.get(d),
        })
    ...
    for i in range(len(quarterly)):
        if i >= 3:
            quarterly[i]["ttm_eps"] = sum(quarterly[j]["eps"] for j in range(i - 3, i + 1))
        else:
            quarterly[i]["ttm_eps"] = None

    for i in range(len(quarterly)):
        if i >= 4:
            prev_eps = quarterly[i - 4]["eps"]
            curr_eps = quarterly[i]["eps"]
            if prev_eps and prev_eps != 0:
                quarterly[i]["yoy"] = (curr_eps - prev_eps) / abs(prev_eps) * 100
            else:
                quarterly[i]["yoy"] = None
        else:
            quarterly[i]["yoy"] = None

    latest = quarterly[-1]
    gross_margin = (latest["gross"] / latest["rev"] * 100) if latest.get("gross") and latest.get("rev") else None
    op_margin = (latest["op"] / latest["rev"] * 100) if latest.get("op") and latest.get("rev") else None
    net_margin = (latest["net"] / latest["rev"] * 100) if latest.get("net") and latest.get("rev") else None
...
def interactive_card_message(
    ...
    *,
    ...
    - E (EPS): Full-width quarterly EPS bar chart with 5-column financial metrics
      (近四季TTM, 最新單季, 單季YoY, 本益比PE, 毛利率).
    ...
    if market:
        ...
        buttons = [
            ...
            _build_nav_pill("EPS", {"type": "message", "label": "EPS", "text": f"E{target}"}, selected in ("E", "EPS")),
    ...
    if selected in ("P", "E", "EPS") and snapshot:
        ...
        if selected in ("E", "EPS") and eps_data:
            ttm_v = eps_data.get("ttm_eps")
            ttm_text = f"近四季累計 EPS {ttm_v:.2f} 元" if ttm_v is not None else "歷季財務報表"
            sub_info = f"{category} ｜ {ttm_text}"
            img_action_url = f"https://tw.stock.yahoo.com/quote/{target}/eps"
            alt_suffix = "EPS 獲利走勢"
        else:
            sub_info = f"{category} · {ts_str}" if ts_str else category
            img_action_url = yahoo_quote_url(target, market=market)
            alt_suffix = "即時走勢"
        ...
        if selected in ("E", "EPS"):
            header_badges.append({
                ...
                "paddingBottom": "2px",
                "contents": [
                    {"type": "text", "text": "EPS 獲利", "size": "xxs", "weight": "bold", "color": "#2563EB"}
                ],
            })
        header_badges.append({"type": "box", "layout": "vertical", "flex": 1, "contents": []})
        header_badges.append({
            "type": "box",
            "layout": "horizontal",
            "flex": 0,
        ...
        if selected in ("E", "EPS") and eps_data:
            latest_eps = eps_data.get("latest_eps")
            yoy_val = eps_data.get("latest_yoy")
            ttm_eps = eps_data.get("ttm_eps")
            gm_val = eps_data.get("gross_margin")
            pe_val = (close / ttm_eps) if close and ttm_eps and ttm_eps > 0 else None

            latest_eps_str = f"{latest_eps:.2f}元" if latest_eps is not None else "--"
            ttm_eps_str = f"{ttm_eps:.2f}元" if ttm_eps is not None else "--"
            yoy_str = f"{yoy_val:+.1f}%" if yoy_val is not None else "--"
            pe_str = f"{pe_val:.1f}倍" if pe_val is not None else "--"
            gm_str = f"{gm_val:.1f}%" if gm_val is not None else "--"

            yoy_color = "#DC2626" if (yoy_val or 0) > 0 else ("#16A34A" if (yoy_val or 0) < 0 else "#111827")
            latest_eps_color = "#DC2626" if (latest_eps or 0) > 0 else ("#16A34A" if (latest_eps or 0) < 0 else "#111827")

            metric_cols = [
                _col_box("近四季TTM", ttm_eps_str, "#111827"),
                {"type": "separator", "color": "#E5E7EB"},
                _col_box("最新單季", latest_eps_str, latest_eps_color),
                {"type": "separator", "color": "#E5E7EB"},
                _col_box("單季YoY", yoy_str, yoy_color),
                {"type": "separator", "color": "#E5E7EB"},
                _col_box("本益比PE", pe_str, "#2563EB"),
                {"type": "separator", "color": "#E5E7EB"},
                _col_box("毛利率", gm_str, "#D97706"),
            ]
        else:
        ...
        return {
            "type": "flex",
            "altText": f"{title} {alt_target} {alt_suffix}",
            "contents": bubble,
        }
...
def handle_command(cmd: ParsedCommand) -> list[dict]:
    ...
    try:
        ...
        if cmd.kind in ("E", "EPS"):
            if str(stock.get("asset_type") or "").upper() == "ETF":
                return [text_message(
                    f"💰 {name} {code}｜ETF\n"
                    "ETF 為指數股票型基金，無個別公司之每股盈餘（EPS）財務報表。\n"
                    f"若欲查詢配息資訊，請輸入：D{code}"
                )]

            eps_data = fetch_stock_eps_data(code)
            if not eps_data or not eps_data.get("quarterly"):
                return [text_message(
                    f"⚠️ {name} {code} 暫無財務報表 EPS 歷季資料"
                )]
            ...
            try:
                ...
                snap = {"name": name, "code": code, "close": None}

            chart = render_eps_sparkline(
                name, code, eps_data["quarterly"], _card_path(f"E_{code}")
            )
            url = make_chart_url(chart)
            return [interactive_card_message(
                url, title=name, target=code, selected="E", market=False, snapshot=snap, eps_data=eps_data
            )] if url else [chart_fallback(chart)]
