...
- Telegram 僅向私有設定檔指定的群組發送通知與查詢回覆。查詢透過獨立長輪詢服務接收，私人／其他群組訊息一律忽略；未啟用付費廣播。詳見 [Telegram 群組查詢](TELEGRAM_GROUP_QUERIES.md)。
- LINE 僅接受已核准且未封存的群組。私人訊息（含原管理員綁定／修改指令）、舊聊天室及未核准群組均不回覆、不推播。盤前／盤後等其他自動推播強制停用。
- LINE 與 Telegram 均有獨立的群組查詢回覆、進出場通知開關。部署保留既有有效開關，不擅自重新開啟 LINE。
