...
# =========================================================
# 若你有自己寫好的推播模組，可以在這裡 import
# 例如：from your_notifier_module import send_telegram_alert
# =========================================================
...
def select_candidates(stocks: list[dict]) -> list[dict]:
    ...
    for stock in sorted(stocks, key=lambda x: num(x.get("amount_rank_pct")) or 0, reverse=True):
        ...
        if amt_rank is not None and amt_rank < 0.35: continue 
...
def main() -> None:
    ...
    print(f"[scan] 啟動波段觸底反彈掃描，候選檔數: {len(candidates)}")
        ...
        try:
            ...
            if df is not None and not df.empty:
                ...
                if result:
                    ...
                    print(f"[hit] ★ {symbol} {stock.get('name', '')} 符合觸底反彈條件！")
    ...
    if buy_list:
        ...
        # 若你原本有寫好的推播模組，可以在這裡呼叫：
        # send_telegram_alert(buy_list)
    else:
        print("[done] 本日無標的符合波段反彈條件。")
